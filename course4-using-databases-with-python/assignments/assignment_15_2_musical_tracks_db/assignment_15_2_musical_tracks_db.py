# You do not need to export or convert the database - simply upload the .sqlite file that your 
# program creates. See the example code for the use of the connect() statement.
...
# The ZIP file contains the tracks.csv file to be used for this assignment. You can export your own tracks 
# from iTunes and create a database, but for the database that you turn in for this assignment, only use the 
# tracks.csv data that is provided. You can adapt the tracks_csv.py application in the zip file to complete 
# the assignment.
...
for line in handle:
    ...
    cur.execute('''INSERT OR REPLACE INTO Track
        ...
        ( name, album_id, length, rating, count, genre_id ) )
                               ...
                               # Fifth line is just a specific format.
...
cur.execute('''SELECT Track.title, Artist.name, Album.title, Genre.name         
        ...
        ON Track.genre_id = Genre.ID and Track.album_id = Album.id
            AND Album.artist_id = Artist.id
        ORDER BY Artist.name LIMIT 3''')
                                                # First line is what we want to see.
                                                # Second line is the tables which hold the data.
                                                # Third and forth line is how the tables are linked.
